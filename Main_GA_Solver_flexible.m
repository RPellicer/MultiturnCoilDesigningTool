...
%     do = x(3)+x(4);  % [METERS] wire outer diameter insulation included
...
%         options =  gaoptimset('PlotFcns',@gaplotbestf,'TolFun',tolerFun,'UseParallel', useparallel); 
    ...
    disp('#######################################################################################################################################################')
    if (tProbe.Coil.Rin ~= tProbe.Coil.Rin_glob)  && tProbe.Coil.emfNear
       warning('3 Main: Inner radius of calculated coil (Rin) is different to specified inner radius (Rin_glob). Please, verify it complies your specifications') 
    end
    StrProbeOpt = getProbeDetails(tProbe,graphs);
    if (StrProbeOpt.Coil.Rin ~= StrProbeOpt.Coil.Rin_glob) && tProbe.Coil.emfNear
       warning('4 Main: Inner radius of calculated coil (Rin) is different to specified inner radius (Rin_glob). Please, verify it complies your specifications') 
    end
...
[s_test,Probe_test] = CalcSensFromDesign(ProbeOpt(1,1,1).Coil.N_v,ProbeOpt(1,1,1).Coil.N_l,ProbeOpt(1,1,1).Coil.do,ProbeOpt(1,1,1).Coil.di,1) ; % (NrLoopsPerLayer,NrLayers,OutWireDiam,CoopDiam, AmplMode)