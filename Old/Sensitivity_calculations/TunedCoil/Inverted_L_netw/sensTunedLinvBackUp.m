...
%%     Structure variables are deployed for visivility
    ...
    dr = tProbe.Coil.dr;
    dz = tProbe.Coil.dz;
    NS = tProbe.Coil.LitzStrandNr_glob; % total number of strands.
    NB = 1;                 % number of bunching operations, (agrupations of strands)
    NC = 0;                 % number of cabling,(agrupations of bunches)
    ...
    RmatchMin = e_n/i_n;  % Usual noise match of the amplifier
    RmatchSearchPointNr = 20;
    if visu == 0
...
%     avoid designs with non-realistic capacitor values
    ...
    probe.Type = 'Tuned';
    if probe.Coil.Rin ~= tProbe.Coil.Rin_glob
       warning('1 sensTuned: Rin different to Rin differes from Rin_glob') 
    end
end