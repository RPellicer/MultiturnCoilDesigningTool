...
di = 0.51e-3; % [METERS] cooper outer diameter
do = 0.584e-3; % [METERS] wire outer diameter insulation included
delta = (do-di)/2; % 0.0001; % [METERS] isolation thickness
a = do/2; % Wire outer radius
% Number of loops estimated by: 1400 = N_v*N_v*(0.035/0.014) #area of the
% coil
% N_v = sqrt(1400/(N_l/N_v))
% N_v = 24; % Number of turns per layer
% N_l = 59; % Number of layers
% dr = 0.0e-3; % axial filling factor = a-sin(pi/3)*a in an hexagonal setup

% To match indutance
N_v = 24; % Number of turns per layer
N_l = 52; % Number of layers
dr = -0.1e-3; % axial filling factor = a-sin(pi/3)*a in an hexagonal setup
dz = 0.0e-3; % azimut filling factor
s = do + sqrt(dr^2+dz^2); % distance between wire centres
R_i = 10e-3; % Coil internal radius
f = 1; %  hexagonal (intercalated)  (f=1)  and orthogonal (f=1/2)
...
vn_out2 = sqrt(en_out2.^2 + ei_out2.^2 + es_out2.^2 + ef_out2.^2);
